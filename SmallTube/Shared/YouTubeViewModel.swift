...
final class YouTubeViewModel: ObservableObject {
    ...
    private func fetchTrending() async {
        do {
            let data = try await NetworkService.fetchYouTube { apiKey in
                URL(string: "https://www.googleapis.com/youtube/v3/videos?part=snippet,contentDetails,statistics&chart=mostPopular&maxResults=\(AppPreferences.resultsCount)&regionCode=\(AppPreferences.countryCode)&key=\(apiKey)")
            }
